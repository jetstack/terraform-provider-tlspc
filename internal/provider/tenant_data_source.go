...
func (d *TenantDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	model.ID = types.StringValue(tenant.ID)
	model.Name = types.StringValue(tenant.Name)
	model.URLPrefix = types.StringValue(tenant.URLPrefix)
	model.Domains, diags = types.ListValueFrom(ctx, types.StringType, tenant.Domains)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
